...
np = K;
L_list = [1 2 5 10].^2; LM = 100; % Number of access points in the network
L_list = [2 4 8 16].^2; LM = 256; % Number of access points in the network. Case LM = 256
...
for Lindx = 1:length(L_list)
    ...
    figure;hold on
    semilogx(L_list, ul_availability);hold on;
    semilogx(L_list, dl_availability);
    grid on;
    savename_ul = ['LM_fixed_UL_' filename '.csv']
    savename_dl = ['LM_fixed_DL_' filename '.csv']
    csvwrite(savename_ul, [L_list' , ul_availability]);
    csvwrite(savename_dl, [L_list' , dl_availability]);
