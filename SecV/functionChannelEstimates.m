function [Hhat,H,B] = functionChannelEstimates(R,nbrOfRealizations,L,K,N,tau_p,pilotIndex,p,ESTIMATOR)
...
%Generate uncorrelated Rayleigh fading channel realizations between each UE
% (K UEs) and all the antennas in the scenario (L*N antennas).
        ...
        for k = find(t==pilotIndex)'
            if ~isempty(k)
                ...
                elseif strcmp(ESTIMATOR, 'LS')
                    RPsi = R(:,:,l,k) / PsiInv; % It's not supposed to be used, but we avoid errors due to lack of defined variables.
                    A_LS = 1/(sqrt(p)*tau_p);
                    Hhat((l-1)*N+1:l*N,:,k) = A_LS*yp;
                end
